...
export class GenerateDialogComponent implements OnInit, OnDestroy {
  ...
  onGenerate() {
    if (this.groupItemSelected.id == 2)
      ...
      this.transaksiService.onGenerateTransaksi(this.itemSelected?.transaksiId, DateUtils.ConvertToTimestampFormatV3(data.dateIn), DateUtils.ConvertToTimestampFormatV3(data.dateOut))
    }
  }
