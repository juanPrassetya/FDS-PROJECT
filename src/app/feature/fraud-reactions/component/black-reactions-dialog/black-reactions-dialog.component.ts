...
import { BlackListState } from '../../../black-list/state/black-list.state';
...
@Component({
  selector: 'app-black-reactions-dialog',
  templateUrl: './black-reactions-dialog.component.html',
  styleUrls: ['./black-reactions-dialog.component.css'],
})
export class BlackReactionsDialogComponent implements OnInit, OnDestroy {
  ...
  form!: FormGroup;
  blackLists: Array<BlackListDomain> = [];
    ...
    this.blackLists$.pipe(takeUntil(this.destroyer$)).subscribe((data) => {
      ...
      if (this.dialogMode == 'EDIT' && data.length > 0) {
        this.getBindingIdField()?.setValue(
          this.blackLists.find((v1) => v1.id == this.itemSelected?.bindingId)
        );
      }
    });
      ...
      ForkJoinHelper(
        ...
        () => {
          if (this.dialogMode == 'EDIT') {
            ...
            if (this.itemSelected?.action == 'ATTR_BLACK_LIST') {
              const existType = JSON.parse(
                this.itemSelected.actionValue
              );
            ...
            this.getBindingTypeField()?.setValue('BLACKLIST');
      ...
      if (this.itemSelected != undefined) {
        ...
        switch (this.itemSelected.action) {
          ...
          case 'ATTR_WHITE_LIST':
            // this.itemSelected.actionValue = data.actionValue?.attribute;
