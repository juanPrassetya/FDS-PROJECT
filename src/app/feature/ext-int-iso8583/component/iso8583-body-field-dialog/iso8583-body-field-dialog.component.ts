...
import {BodyFieldAdd, BodyFieldUpdate} from "../../state/ext-int-iso8583.actions";
import {
...
@Component({
  selector: 'app-iso8583-body-field-dialog',
  templateUrl: './iso8583-body-field-dialog.component.html',
  styleUrls: ['./iso8583-body-field-dialog.component.css']
})
export class Iso8583BodyFieldDialogComponent implements OnInit, OnDestroy {
  @Input() isOpen: boolean = true
  @Input() dialogMode: string = 'ADD'
  @Input() itemSelected!: BodyFieldDomain | undefined
  ...
  ngOnInit() {
    ...
    this.action$
      .pipe(
        ofActionSuccessful(BodyFieldAdd, BodyFieldUpdate),
        takeUntil(this.destroyer$)
      ).subscribe(() => { this.onClose() })

    // this.action$
    //   .pipe(
    //     ofActionCompleted(BodyFieldAdd, BodyFieldUpdate),
    //     takeUntil(this.destroyer$)
  ...
  onSave(data: any) {
    if (this.msgConfiguration != undefined) {
      ...
      if (this.dialogMode == 'EDIT') {
        ...
        this.extIntISO8583Service.onUpdateBodyField(data)
      } else this.extIntISO8583Service.onAddBodyField(data)
    }
  }
